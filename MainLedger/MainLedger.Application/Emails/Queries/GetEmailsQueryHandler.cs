...
public class GetEmailsQueryHandler : IRequestHandler<GetEmailsQuery, PaginatedResponse<EmailListItemDto>>
{
    ...
    public async Task<PaginatedResponse<EmailListItemDto>> Handle(GetEmailsQuery request, CancellationToken cancellationToken)
    {
        _logger.LogInformation(
            "Getting emails for user {UserId}: Page={Page}, PageSize={PageSize}, Status={Status}, IsFinancial={IsFinancial}",
            request.UserId, request.Page, request.PageSize, request.Status, request.IsFinancial);
        ...
        // Map to DTOs
        var items = emails.Select(email => new EmailListItemDto
        {
            Id = email.Id,
