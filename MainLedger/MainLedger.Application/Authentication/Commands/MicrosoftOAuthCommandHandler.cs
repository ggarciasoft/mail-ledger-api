...
public class MicrosoftOAuthCommandHandler : IRequestHandler<MicrosoftOAuthCommand, LoginResult>
{
    ...
    private readonly ILogger<MicrosoftOAuthCommandHandler> _logger;

    public MicrosoftOAuthCommandHandler(
        ...
        ILogger<MicrosoftOAuthCommandHandler> logger
    )
    {
    ...
    public async Task<LoginResult> Handle(MicrosoftOAuthCommand request, CancellationToken cancellationToken)
    {
        _logger.LogInformation("Microsoft OAuth authentication attempt");

        // Exchange code for user info
        var userInfo = await _oauthService.ExchangeCodeForUserInfoAsync("microsoft", request.Code, cancellationToken);
        ...
        if (externalLogin != null)
        {
            ...
            _logger.LogInformation("Existing Microsoft SSO user {UserId} logged in", user.Id);
        }
        else
        {
            ...
            if (existingUser != null)
            {
                ...
                _logger.LogInformation("Linked Microsoft account to existing user {UserId}", user.Id);
            }
            else
            {
                ...
                _logger.LogInformation("Created new user {UserId} via Microsoft SSO", user.Id);
            }
        }
        ...
        _logger.LogInformation("User {UserId} authenticated via Microsoft OAuth successfully", user.Id);
