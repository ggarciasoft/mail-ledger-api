...
public static class DatabaseSeeder
{
    ...
    public static async Task SeedAsync(MailLedgerDbContext context)
    {
        ...
        if ((await context.ExtractionVersions.AnyAsync()) == false)
        {
            // Create default extraction version
            var defaultExtractionVersion = ExtractionVersion.Create(
                "v1.0",
                "gpt-3.5-turbo",
                "initial-prompt-hash"
            );

            context.ExtractionVersions.Add(defaultExtractionVersion);
            await context.SaveChangesAsync();
        }
    }
}
